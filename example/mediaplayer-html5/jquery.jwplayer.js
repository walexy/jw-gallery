...
(function($) {
	...
	function buildHandler(element, handler, player, args) {
		var nam = player.id + '_' + element;
		$('#' + player.id).unbind('canplay').bind( 'canplay',function (){
	...
	function resizeHandler(event) {
		...
		if ( (event.fullscreen) ) {
			...
			var domelement = document.getElementById( event.id );
			if ( domelement.webkitSupportsFullscreen ){
				domelement.webkitEnterFullscreen();
				controlbars[event.id].fullscreen = false;
			} else {
			...
			$('#' + event.id )
				...
				.css("width", event.width)
				.css("height", event.height)
	...
	// 	_timeSliderBuffer, timeSliderRail some percent
		var el = $('#' + event.id + '_jwplayerControlbar');
...
(function($) {
	...
	function setState(player, newstate) {
		var playerID = player.model.domelement.attr('id');
		...
		if (newstate == $.fn.jwplayer.states.IDLE) {
			...
			if (player.model.domelement.css('display') != 'none') {
				player.model.domelement.css({'display': 'block'}); // added fix
				$("#" + playerID + "_displayImage").css({opacity:'1'}); // added enhancement
			}
		}
	...
	function stop(player) {
		try{
			...
			player.model.domelement[0].stop();
		};
		}catch(err){
...
(function($) {
	...
	$.fn.jwplayerView.embedFlash = function(player, options) {
		if (player.model.config.flashplayer) {
			...
			// added condition to check and see if config.file has been set
			if ((!$.fn.jwplayerUtils.isNull(player.model.sources[player.model.source])) && ($.fn.jwplayerUtils.getAbsolutePath(config.file)==null)){
